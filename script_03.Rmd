...
Decide what data is actually relevant to the problem we want to solve.
Choose a relevant estimand - change in undergrad enrollment per class.
Consider what data you may want to acquire - we may want to compare the number 
of classes students are taking to the number of students that are enrolled.

After we have a model, Temperance suggests we should:

There is uncertainty around why students might be dropping classes - it could be
that they're dropping a fifth class, taking a leave of absence, or switching
courses. We need to understand our model in context if we ever use this 
model for comparison. Our model is a simplified version of reality. 
...
sep_total <- inner_join (sep_old, sep_new, by = c("id", "name", "title", "department")) %>%
  mutate(diff = u_grad_old - u_grad_new) %>%
  arrange(desc(diff))
